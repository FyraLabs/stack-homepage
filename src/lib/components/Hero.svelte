<!-- eslint-disable-next-line svelte/no-navigation-without-resolve -->
<main class="px-6 pt-14 lg:px-8 relative isolate overflow-hidden border-b border-fyra-gray-800">
	<!-- Top blob -->
	<div
		aria-hidden="true"
		class="inset-x-0 -top-40 blur-3xl sm:-top-80 absolute -z-10 transform-gpu overflow-hidden"
	>
		<div
			style="clip-path: polygon(74.1% 44.1%, 100% 61.6%, 97.5% 26.9%, 85.5% 0.1%, 80.7% 2%, 72.5% 32.5%, 60.2% 62.4%, 52.4% 68.1%, 47.5% 58.3%, 45.2% 34.5%, 27.5% 76.7%, 0.1% 64.9%, 17.9% 100%, 27.6% 76.8%, 76.1% 97.7%, 74.1% 44.1%)"
			class="w-144.5 sm:left-[calc(50%-30rem)] sm:w-288.75 relative left-[calc(50%-11rem)] aspect-1155/678 -translate-x-1/2 rotate-30 bg-linear-to-tr from-fyra-red-950 to-fyra-red-700 opacity-25"
		></div>
	</div>

	<div class="max-w-4xl py-32 sm:py-40 mx-auto">
		<div class="text-center">
			<h1 class="text-3xl font-semibold tracking-tight sm:text-6xl text-balance text-fyra-gray-50">
				Infrastructure without breaking the bank, <span class="text-fyra-red-400"
					>or your mind.</span
				>
			</h1>
			<p
				class="font-base mt-8 text-base sm:px-16 sm:text-xl/7 lg:px-32 text-pretty text-fyra-gray-400"
			>
				Built from the ground up to support the open-source projects we love, and the communities
				around them.
			</p>
		</div>
	</div>

	<!-- Bottom blob -->
	<div
		aria-hidden="true"
		class="inset-x-0 blur-3xl sm:top-[calc(100%-30rem)] absolute top-[calc(100%-13rem)] -z-10 transform-gpu overflow-hidden"
	>
		<div
			style="clip-path: polygon(74.1% 44.1%, 100% 61.6%, 97.5% 26.9%, 85.5% 0.1%, 80.7% 2%, 72.5% 32.5%, 60.2% 62.4%, 52.4% 68.1%, 47.5% 58.3%, 45.2% 34.5%, 27.5% 76.7%, 0.1% 64.9%, 17.9% 100%, 27.6% 76.8%, 76.1% 97.7%, 74.1% 44.1%)"
			class="w-144.5 sm:left-[calc(50%+36rem)] sm:w-288.75 relative left-[calc(50%+3rem)] aspect-1155/678 -translate-x-1/2 bg-linear-to-tr from-fyra-red-950 to-fyra-red-700 opacity-25"
		></div>
	</div>
</main>
